...
const NavDropdown: React.FC<{
  ...
  return (
    <div className="relative group/nav h-full flex items-center">
      <button 
        className={`flex items-center gap-1.5 text-sm font-bold py-2 px-3.5 rounded-xl transition-colors relative ${
          isCategoryActive 
      ...
      >
        ...
        {isCategoryActive && (
          <span className="absolute -bottom-1 left-3 right-3 h-0.5 bg-accent dark:bg-indigo-400 rounded-full shadow-[0_0_8px_rgba(79,70,229,0.6)]"></span>
        )}
      </button>

      {/* 下拉菜单：使用针对性过渡，避免初始可见 */}
      <div className="invisible group-hover/nav:visible opacity-0 group-hover/nav:opacity-100 absolute top-full left-1/2 -translate-x-1/2 pt-2 min-w-[260px] z-[60] transition-[opacity,transform] duration-200 transform translate-y-2 group-hover/nav:translate-y-0 pointer-events-none group-hover/nav:pointer-events-auto">
...
const TopNav: React.FC<TopNavProps> = ({ 
  ...
  return (
    <>
        ...
        {/* 移动端菜单：通过 isMounted 状态控制初始不执行过渡动画 */}
        <div 
            className={`fixed inset-0 z-[100] ${isMobileSidebarOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} ${isMounted ? 'transition-opacity duration-300' : ''}`}
        >
            <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setIsMobileSidebarOpen(false)} />
            <div className={`absolute top-0 right-0 h-full w-[80%] max-w-sm bg-white dark:bg-d-primary shadow-2xl ${isMounted ? 'transition-transform duration-300 transform' : ''} ${isMobileSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
