...
const CopyButton: React.FC<{ text: string }> = ({ text }) => {
    ...
    return (
        <button
            ...
            className="p-1.5 text-text-secondary dark:text-d-text-secondary hover:bg-border-color dark:hover:bg-d-border-color rounded-md transition-colors"
        >
...
const ResultRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
    <div className="flex items-center justify-between p-3 bg-primary dark:bg-d-primary rounded-md">
        <span className="font-semibold text-text-secondary dark:text-d-text-secondary">{label}</span>
        <div className="flex items-center gap-2">
            <code className="text-accent dark:text-d-accent break-all">{value}</code>
    ...
    return (
        <div>
            ...
            <input
                ...
                className="w-full p-4 mb-8 text-lg font-mono bg-secondary dark:bg-d-secondary border border-border-color dark:border-d-border-color rounded-lg focus:outline-none focus:ring-2 focus:ring-accent dark:focus:ring-d-accent"
                aria-label="URL Input"
            />

            <h3 className="text-xl font-bold text-text-primary dark:text-d-text-primary mb-4">{t('tools.urlParser.resultsTitle')}</h3>
            <div className="p-6 bg-secondary dark:bg-d-secondary border border-border-color dark:border-d-border-color rounded-lg space-y-3">
                {error && !parsedUrl && (
                ...
                {parsedUrl && (
                    <>
                        ...
                        {parsedUrl.params.length > 0 && (
                             <div className="pt-4">
                                <h4 className="font-semibold text-text-secondary dark:text-d-text-secondary mb-2">{t('tools.urlParser.queryParams')}</h4>
                                <div className="space-y-2 p-3 bg-primary dark:bg-d-primary rounded-md">
                         ...
                         {parsedUrl.params.length === 0 && (
                             <p className="text-sm text-text-secondary dark:text-d-text-secondary py-2">{t('tools.urlParser.noParams')}</p>
                         )}
                    </>
                )}
            </div>
